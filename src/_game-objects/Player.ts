...
import { Vector2 } from 'src/_types/common';
export class Player{
    spine: any;
    hands: any;
    handBones: any = [];
    ...
    init(options: {parent: Phaser.Scene, graphics: Phaser.GameObjects.Graphics}){
    ...
    update(options: any){
        ...
        //     else if(key == 'cursors') cursors = value;
        //     else if(key == 'mousePos') mousePos = value;
        //     else{
    ...
    play(animation: string, loop = false){
    ...
    lerp(start: number, target: number, amount: number){
